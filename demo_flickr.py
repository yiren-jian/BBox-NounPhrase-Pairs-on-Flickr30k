...
if __name__ == '__main__':
    ...
    phrase_ids = list(set(phrase_ids))
    ...
    all_sentence_list = []
    all_sentence_colors = []
    for caption in captions:   #### 3 captions per image, for example
        no_caption += 1
    ...
    for phrase_id in anns['boxes']:    #### phrase_id is the id for an object
        ...
        for bbox in bboxes:
            ...
            with torch.no_grad():
                zeroshot_weights = []
                for classname in tqdm(classnames):   ### from all COCO classes
                ...
                _, idxs = logits.topk(k=10)
                idxs = list(idxs.cpu().numpy()[0])
                to_remove_classname = [classnames[idx] for idx in idxs]
                print("to_remove_classname", to_remove_classname)

            matched_np_with_probs = dict()     ### multiple nps can match to a bbox
            for np in nps:
                # classnames.remove(category_name)     #### remove the target class name

                ##### start another iteration
                classnames = copy.copy(nms)
                classnames = [item for item in classnames if item not in to_remove_classname]   #### remove the top10 in the first run
                classnames = [np] + classnames    #### put noun phrase at idx 0
                with torch.no_grad():
                    zeroshot_weights = []
                    for classname in tqdm(classnames):   ### from all COCO classes
                        texts = [template.format(classname) for template in imagenet_templates] #format with class
                ...
                # if probs > 0.4:      ############ method 1   ############# is giving a "hard" threshold
                #     if np == predicted_classname:
                #         print("assign a matching between noun phrase and this box")
                #         if np in phrase_id2phrases[phrase_id]:
                #             print("a correct matching :)")
                #             correct_count += 1
                #             correct_matched_nps.append(np)
                #             correct_predicted_bboxes_coordinate.append((bbox[0], bbox[1], bbox[2], bbox[3]))
                #         else:
                #             print("a wrong matching :(")
                #             wrong_count += 1
                #             if np not in wrong_matched_nps:
                #                 wrong_matched_nps[phrase_id] = [np]
                #             else:
                #                 wrong_matched_nps[phrase_id].append(np)


                if probs > 0.0:          ############ method 2 ############  is comparing probs between matched nps.
                    if np == predicted_classname:
                        matched_np_with_probs[np] = probs     ############ method 2 ############  is comparing probs between matched nps.

            if len(matched_np_with_probs) !=0:     ############ method 2 ############  is comparing probs between matched nps.
                max_probs = 0
                for k, v in matched_np_with_probs.items():
                    if matched_np_with_probs[k] > max_probs:
                        max_probs = matched_np_with_probs[k]
                        best_np = k

                if max_probs > 0.4:
                    print("assign a matching between noun phrase and this box")     ############ method 2 ############  is comparing probs between matched nps.
                    if best_np in phrase_id2phrases[phrase_id]:
                        print("a correct matching :)")
                        print("with ", best_np)
                        correct_count += 1
                        correct_matched_nps.append(best_np)
                        correct_predicted_bboxes_coordinate.append((bbox[0], bbox[1], bbox[2], bbox[3]))
                    else:
                        print("a wrong matching :(")
                        print("with ", best_np)
                        wrong_count += 1
                        if np not in wrong_matched_nps:
                            wrong_matched_nps[phrase_id] = [best_np]
                        else:
                            wrong_matched_nps[phrase_id].append(best_np)    ############ method 2 ############  is comparing probs between matched nps.
    ...
    new_sentence_colors = sentence_colors[0].copy()
    background_color = sentence_colors[1].copy()
    ...
    #     bboxes = anns['boxes'][phrase_id]   ### get bboxes ( maybe > 1) for an object class
    #     for bbox in bboxes:
    #         if phrase_id in wrong_matched_nps.keys():
    #             for ii in range(len(wrong_matched_nps[phrase_id])):
    #                 xxx = wrong_matched_nps[phrase_id][ii]
    #                 xxx_id = phrase_text2phrase_id[xxx]   #### xxx is a np, that this bbox wrongly matches to
    #                 bbox_color = colors[phrase_id2score[xxx_id]+1]    ##### change the color of this bbox to the "np" that it wrongly matches to
    #                 start_point = (round(bbox[0]), round(bbox[1]))
    #                 end_point = (round(bbox[2]), round(bbox[3]))
    #                 plt.gca().add_patch(Rectangle(start_point, round(bbox[2])-round(bbox[0]), round(bbox[3])-round(bbox[1]),
    #                         edgecolor=bbox_color,
    #                         hatch=random.sample(hatches, k=1)[0],
    #                         linestyle = '-',
    #                         facecolor='none',
    #                         lw=0))
