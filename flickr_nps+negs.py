...
if __name__ == '__main__':
    ...
    for phrase_id in anns['boxes']:    #### phrase_id is the id for an object
        ...
        for bbox in bboxes:
            ...
            #
            # classnames = copy.copy(nms)
            # classnames = [item for item in classnames if item not in to_remove_classname]   #### remove the top5 in the first run
            # classnames = nps + classnames[0:5]    #### put noun phrase at idx 0

            classnames = nps + random.sample(classnames, k=5)
            with torch.no_grad():
                zeroshot_weights = []
