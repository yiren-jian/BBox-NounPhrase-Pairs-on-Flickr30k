#############################################################
########## This version will save coordinates of boxes ONLY
#############################################################
...
if __name__ == '__main__':
    ...
    flickr30k_path = 'boxes/'
    filenames = [f for f in listdir(flickr30k_path) if isfile(join(flickr30k_path, f)) if f.endswith('pkl')]
    ...
    print("number of images in Flickr: ", len(image_ids))

    to_do_list = [item[0].split('/')[-1].split('.')[0] for item in image_ids[args.start:args.end]]

    def run_one_image(image_ids, found_list, args, iii):
        image_filename, image_id = image_ids[iii][0], image_ids[iii][1]
        image_name = image_filename.split('/')[-1].split('.')[0]
        if image_name in found_list:
        ...
        all_boxes_coordinates = []
        for noun in all_matched_nps_boxes_dict:
            for bbox in all_matched_nps_boxes_dict[noun]:
                my_bbox = [float(bbox[0]), float(bbox[1]), float(bbox[2]), float(bbox[3])]
                if my_bbox not in all_boxes_coordinates:
                    all_boxes_coordinates.append(my_bbox)
                    final_num_selected_boxes += 1

        with open('boxes/%s.pkl'%(image_name), 'wb') as f:
            pickle.dump(all_boxes_coordinates, f)
    ...
    for iii in tqdm(range(args.start, args.end)):
        start = time.time()
        try:
            run_one_image(image_ids, found_list, args, iii)
        except:
            with open("failures-%d.txt"%args.run, "a") as f:
                to_write_down = "failed in iteration: %d, %s \n"%(iii, image_ids[iii][0])
                f.write(to_write_down)
            pass

        end = time.time()
        with open("times-%d.txt"%args.run, "a") as f:
            to_write_down = "%d, %.2f \n"%(iii, end-start)
            f.write(to_write_down)
